...
6. Subdivisions are hidden by default. Select a country and use **Show Subdivisions / Hide Subdivisions** to control their visibility, including newly created subdivisions. Changing countries or closing the country panel hides its subdivisions without deleting them. Click a visible subdivision to rename, recolor, or delete it; close its panel to return to the country controls.
   ...
   When country borders change, affected subdivisions are clipped to their country's remaining territory. This also applies when creating or expanding another country with overlapping disabled. A subdivision can split into separate pieces while keeping its name, color, and ID. If it would lose all its territory, AtlasShift asks before deleting it. Cancelling keeps all committed country borders and subdivisions unchanged, and leaves the draft available for revision.

   To reshape a subdivision, click it and select **Edit Borders**. Drag vertices, click a border segment to insert a vertex, or right-click a vertex to remove it. Each ring must retain at least three vertices. **Done** clips the result to the parent country and applies the overlap setting to sibling subdivisions, with confirmation before complete absorption. **Cancel Border Edit** restores the previous boundary. Invalid or self-crossing boundaries are rejected; a boundary moved entirely outside the country is reset to its original shape. Finish or cancel the boundary edit before saving or changing the overlap setting. Separate polygon pieces and interior holes are supported.
...
```json
{
  "country-a": { "name": "New display name", "color": "#123456" }
}
```

Loading older worlds maps their name-based edits to feature IDs, including custom countries and explicit deletions. Exact duplicate features without IDs are removed from legacy maps; distinct same-name features and sea masks are retained. When several distinct legacy features share a name, the old edit applies to each, preserving the previous name-based behavior. New imports keep same-name features independent.
...
- **Subdivision editing:** subdivisions support vertex editing, visibility controls, parent-country clipping, and overlap trimming between siblings. Moving a shared border does not move the neighboring subdivision's vertices in tandem; shrinking a region can leave an unassigned gap. Subdivision GeoJSON import/export is not implemented yet.
...
Future versions of AtlasShift are planned to expand country subdivisions such as states, provinces, regions, and departments. The editor already supports creating, naming, coloring, deleting, saving, and reloading subdivisions with parent-country relationships, vertex editing, visibility controls, and sibling-overlap management. Next steps include import/export support for subdivision datasets and coordinated editing of shared borders.
